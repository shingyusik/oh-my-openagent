...
| P2 | **Project-adaptive agents.** 코어 에이전트는 고정하되, 워커 프로필과 코드 컨벤션은 프로젝트가 실제로 쓰며 채택·폐기·진화한다. | 책임 경계는 유지하면서 도메인/스택 고정 방지 |
...
| P12 | **Maestro is the only user surface and decision owner.** 사용자는 Maestro Core와만 대화한다. Maestro Core는 최종 판단, HITL 질문, 실행 승인을 소유하고 PM 반복 노동은 private PM sub-agent에 위임한다. 모든 sub-agent 입출력은 구조화 산출물로만 Maestro Core에 반환된다. | 사용자 인지부담 일원화 + Maestro Core 과부하 방지 + 컨텍스트 격리 |
...
P1 (Plan-first)  +  P14 (4-step) +  P15 (DoD)
   └─ Maestro Core의 Planning 단계에서 한꺼번에 강제

P3 (Context isolation) + P12 (Single surface)
   └─ Maestro Core만 user-facing, private PM sub-agent와 워커는 task() 격리
...
P2 (Project-adaptive agents) + P11 (Self-evolving)
   └─ worker profiles / code conventions / management rules are proposed, measured, accepted, or deprecated
...
| 원칙 | 강제 메커니즘 |
|---|---|
| P1 | Maestro Core가 plan 없이 dispatch 거부 |
...
| P12 | Foreman/Sentinel/L2 워커/private PM sub-agent는 user-facing 출력 채널 없음 (시스템 차원). Maestro Core는 board 본문 전체 대신 `_index.md` + active item + 요약만 상시 보유 |
