...
### 변경 요약
- **정식명 확정**: 현재 설계 중인 하네스 이름을 `VibeForge Harness`로 정함.
- **한 줄 정의 추가**: "바이브코딩 기법을 구조화하고, 프로젝트 경험으로 계속 진화하는 하네스"를 설계 진입 문서에 반영.
- **현재형 문서 톤 정리**: 루트 README와 overview에서 범용 기능명 중심 표현을 제품명 중심 설명으로 교체.
